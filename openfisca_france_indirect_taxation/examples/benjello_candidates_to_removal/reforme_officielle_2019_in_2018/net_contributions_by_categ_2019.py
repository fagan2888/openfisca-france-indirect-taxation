...
log.info((df_reforme['pondmen'] * df_reforme['ticpe_totale_officielle_2019_in_2018']).sum() / 1e9)
...
for category in ['niveau_vie_decile']:  # ['niveau_vie_decile', 'age_group_pr', 'strate']:
    ...
    log.info((df_reforme['cout_reforme_cheque_officiel'] * df_reforme['pondmen']).sum() / 1e6)
    log.info((df_reforme['cout_reforme_cheque_majore'] * df_reforme['pondmen']).sum() / 1e6)
    log.info((df_reforme['cout_reforme_cheque_philippe'] * df_reforme['pondmen']).sum() / 1e6)

    log.info((df_reforme['cheques_energie_officielle_2019_in_2018'] * df_reforme['pondmen']).sum() / 1e6)
    log.info((df_reforme['cheques_energie_majore_officielle_2019_in_2018'] * df_reforme['pondmen']).sum() / 1e6)
    log.info((df_reforme['cheques_energie_philippe_officielle_2019_in_2018'] * df_reforme['pondmen']).sum() / 1e6)
